...
import config
from Esproritik import LOGGER, app, userbot
from Esproritik.core.call import Loy
from Esproritik.misc import sudo
from Esproritik.plugins import ALL_MODULES
from Esproritik.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("Esproritik.plugins" + all_module)
    LOGGER("Esproritik.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Loy.start()
    try:
        await Loy.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("Esproritik").error(
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("Esproritik").info("Stopping Espro ritik Bot...")
